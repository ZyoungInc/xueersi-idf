...
void cdc_acm_custom_send(usb_dev *udev, uint8_t *buffer, uint32_t length)
{
    usb_cdc_handler *cdc = (usb_cdc_handler *)udev->dev.class_data[CDC_COM_INTERFACE];

    if (length == 0) return;

    if(cdc->packet_sent == 0) return;  // 防止重入

    cdc->packet_sent = 0U;
    usbd_ep_send(udev, CDC_DATA_IN_EP, buffer, length);
}
...
extern void cdc_acm_data_receive_callback(uint8_t *buffer, uint32_t length);
extern void esp32_reset_control(uint8_t dtr, uint8_t rts);
...
static uint8_t cdc_acm_req(usb_dev *udev, usb_req *req)
{
    ...
    case SET_CONTROL_LINE_STATE:
        {
            uint16_t control_line_state = req->wValue;
            // 提取 DTR (bit 0) 和 RTS (bit 1)
            uint8_t dtr = (control_line_state & 0x01) ? 1 : 0;
            uint8_t rts = (control_line_state & 0x02) ? 1 : 0;
            
            /* 【重要修复】：恢复调用下载时序控制代码 */
            esp32_reset_control(dtr, rts);
        }
        break;
...
static uint8_t cdc_acm_ctlx_out(usb_dev *udev)
{
    ...
    if(NO_CMD != udev->dev.class_core->alter_set) {
        ...
        /* ========================【核心修复：USART0 改为 USART1】======================== */
        uint32_t baud_rate = cdc->line_coding.dwDTERate;
        // 当上位机切换波特率时（如 esptool 从 115200 切换到 921600），动态调整 GD32 硬件串口1的波特率
        if ((baud_rate >= 1200) && (baud_rate <= 2000000)) {
...
static uint8_t cdc_acm_out(usb_dev *udev, uint8_t ep_num)
{
    ...
    if (cdc->receive_length > 0) {
        cdc_acm_data_receive_callback((uint8_t *)(cdc->data), cdc->receive_length);
    }

    /* 手动释放硬件状态机，允许下一包的接收 */
    cdc->packet_receive = 1U;
    // cdc->packet_sent = 1U;  <----- ?【必须删掉这行】OUT 端点接收完成，不能去改变 IN 端点的状态！
